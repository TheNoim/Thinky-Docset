...
_Example:_ Each user has exactly one account, and each account belongs to exactly one user.  
```javascript
...
_Example:_ An author has many posts, but each post is written by only one author.
